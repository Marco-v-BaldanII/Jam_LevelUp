...
public class CottonMines : wolf_task
{
    ...
    public override void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.gameObject.CompareTag("Wolf") == true)
        {
            ...
            if (wolf != null && wolf.has_cotton == false && wolf.my_mood == Wolf_Mood.NORMAL && wolf.my_state != Wolf_State.PLAYING && wolf.my_mood != Wolf_Mood.ENRAGED && wolf.my_state != Wolf_State.WALKING_TO_NOTHING) // Also check if wolf is not already in the list
            {
                current_wolf = wolf;
